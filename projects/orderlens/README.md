...
OrderLens는 판매 경로와 주문 번호를 묶어 주문을 구분하고, 버전별 변경 내역을 저장합니다. 집계할 때는 조회 시점에 해당하는 최신 버전만 사용합니다. 판매 경로별 지연 주문 조회와 운영 문서 검색도 제공합니다.
...
현재 버전은 **0.1.1**입니다. SQLite에서 실행하며, PostgreSQL·Docker 설정은 실제 통합 검증이 남아 있습니다. 실제 고객 데이터, 외부 배포, LLM 답변 생성은 아직 포함하지 않았습니다.

다음 변경은 주문 목록의 페이지 처리와 PostgreSQL 통합 검증입니다. [작업 목록](docs/automation_and_backlog.md) · [개발 기록](docs/progress.md) · [참고 저장소](docs/references.md) · [기여 안내](CONTRIBUTING.md)
